...
<script type="text/javascript">
    ...
    $("#banner_img").resizeImg({

        callback: function (base64str) {

            var val = $('#banner_img').val().toLowerCase();
            var ext = val.split('.')[1];
            var docName = val.split('fakepath')[1];
            var filename = $('#banner_img').val().replace(/C:\\fakepath\\/i, '')
    ...
    $(".create_banner_btn").click(function () {
        ...
        if (title == '') {
            $(".error_top").show();
            $(".error_top").html("");
            $(".error_top").append("<p>{{trans('lang.title_banner_error')}}</p>");
            window.scrollTo(0, 0);
