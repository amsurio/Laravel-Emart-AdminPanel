...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        ...
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">{{trans('lang.live_tracking_parcel')}}</h3>
        </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var orders = [];
            var orders_drivers = [];
            database.collection('parcel_orders').where('status', '==', 'In Transit').get().then(async function (snapshots) {
            ...
            var drivers = [];
            database.collection('users').where('role', '==', 'driver').where('serviceType', '==', 'parcel_delivery').where('location', '!=', null).get().then(async function (snapshots) {
                if (snapshots.docs.length > 0) {
                    snapshots.docs.forEach((doc) => {
                        var data = doc.data();
                        data.flag = 'available';
                ...
                if(driver!='' && driver != undefined){
                    if (val.flag == "in_transit") {
                        ...
                        if (user != undefined && user!='') {
                            ...
                            if (val.author.shippingAddress && val.sender && val.senderLatLong) {
                                html += '<div class="location-ride">';
                                html += '<div class="from-ride"><span>' + val.sender.address + '</span></div>';
                            ...
                            html += '&nbsp;&nbsp;<a href="/parcel_orders/edit/' + val.id + '" class="badge badge-info" target="_blank">{{trans("lang.order_id")}} : ' + val.id.substring(0, 7) + '</a>';
                            html += '</div>';
                            html += '</div>';
