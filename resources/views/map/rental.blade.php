...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        ...
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">{{trans('lang.live_tracking_rental')}}</h3>
        </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var orders = [];
            var orders_drivers = [];
            database.collection('rental_orders').where('status', '==', 'In Transit').get().then(async function (snapshots) {
            ...
            var drivers = [];
            database.collection('users').where('role', '==', 'driver').where('serviceType', '==', 'rental-service').where('location', '!=', null).get().then(async function (snapshots) {
                if (snapshots.docs.length > 0) {
                    snapshots.docs.forEach((doc) => {
                        var data = doc.data();
                        data.flag = 'available';
                ...
                if(driver!='' && driver != undefined){
                    ...
                    if (val.flag == "in_transit") {
                        ...
                        if (user != undefined && user!='') {
                            ...
                            if (val.author.shippingAddress && val.dropAddress) {
                                html += '<div class="location-ride">';
                                html += '<div class="from-ride"><span>' + val.dropAddress + '</span></div>';
