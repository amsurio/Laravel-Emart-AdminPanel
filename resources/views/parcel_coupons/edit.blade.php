...
<div class="page-wrapper">
  <div class="row page-titles">
    <div class="col-md-5 align-self-center">
      <h3 class="text-themecolor">{{trans('lang.parcel_plural')}} {{trans('lang.coupon_plural')}}</h3>
    </div>
    ...
    <div class="col-md-7 align-self-center">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{!! route('parcel_coupons') !!}">{{trans('lang.parcel_plural')}}
    ...
    <div>
      ...
      <div class="form-group col-12 text-center btm-btn">
        ...
        <a href="{!! route('parcel_coupons') !!}" class="btn btn-default"><i
...
<script>
  ...
  $(document).ready(function () {
    ...
    ref.get().then(async function (snapshots) {
      var parcel_coupons = snapshots.docs[0].data();



      if (parcel_coupons.image != '' && parcel_coupons.image != null) {
        photo = parcel_coupons.image;
        oldImageFile = parcel_coupons.image;
      ...
      $(".coupon_code").val(parcel_coupons.code);
      $("#coupon_discount_type").val(parcel_coupons.discountType);
      $(".coupon_discount").val(parseInt(parcel_coupons.discount));
      $(".coupon_description").val(parcel_coupons.description);

      if (parcel_coupons.isEnabled) {
        $(".coupon_enabled").prop("checked", true);
      }


      if (parcel_coupons.hasOwnProperty("expiresAt")) {

        try {
          var date1 = parcel_coupons.expiresAt.toDate().toDateString();
  ...
  $(".save_coupon_btn").click(function () {
    ...
    database.collection('parcel_coupons').where('id','!=',id).where('code', '==', code).get().then(async function (snapshot) {
      ...
      if (code == '') {
        ...
        storeImageData().then(IMG => {
          database.collection('parcel_coupons').doc(id).update({ 'code': code, 'description': description, 'discount': discount, 'expiresAt': expiresAt, 'isEnabled': isEnabled, 'id': id, 'discountType': discountType, 'image': photo }).then(function (result) {
            window.location.href = '{{ route("parcel_coupons")}}';
          });
