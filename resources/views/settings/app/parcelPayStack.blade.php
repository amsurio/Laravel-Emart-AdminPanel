        ...
        <div class="card">
            <div class="payment-top-tab mt-3 mb-3">
                <ul class="nav nav-tabs card-header-tabs align-items-end">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active parcel_payfast_active_label"
...
$(document).ready(function () {
    ...
    parcelPayStack.get().then(async function (snapshots) {
        var parcelPayStack = snapshots.data();

        if (parcelPayStack == undefined) {
            database.collection('settings').doc('parcelPayStack').set({});
        }

        try {
            if (parcelPayStack.isEnable) {
            ...
            if (parcelPayStack.isSandbox) {
                $(".sand_box_mode").prop('checked', true);
            }

            $(".secretKey").val(parcelPayStack.secretKey);
            $(".publicKey").val(parcelPayStack.publicKey);
            $(".callbackURL").val(parcelPayStack.callbackURL);
            $(".webhookURL").val(parcelPayStack.webhookURL);
    ...
    $(".save_paystack_btn").click(function () {
        ...
        database.collection('settings').doc("parcelPayStack").update({
            'secretKey': secretKey,
            'publicKey': publicKey,
            'isEnable': isEnable,
            'isSandbox': isSandbox,
            ...
            window.location.href = '{{ url("settings/payment/parcelPayStack")}}';
