        ...
        <div class="card">
            <div class="payment-top-tab mt-3 mb-3">
                <ul class="nav nav-tabs card-header-tabs align-items-end">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active rental_payfast_active_label"
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        rentalPayStack.get().then(async function (snapshots) {
            var rentalPayStack = snapshots.data();

            if (rentalPayStack == undefined) {
                database.collection('settings').doc('rentalPayStack').set({});
            }

            try {
                if (rentalPayStack.isEnable) {
                ...
                if (rentalPayStack.isSandbox) {
                    $(".sand_box_mode").prop('checked', true);
                }

                $(".secretKey").val(rentalPayStack.secretKey);
                $(".publicKey").val(rentalPayStack.publicKey);
                $(".callbackURL").val(rentalPayStack.callbackURL);
                $(".webhookURL").val(rentalPayStack.webhookURL);
        ...
        $(".save_paystack_btn").click(function () {
            ...
            database.collection('settings').doc("rentalPayStack").update({
                'secretKey': secretKey,
                'publicKey': publicKey,
                'isEnable': isEnable,
                'isSandbox': isSandbox,
                ...
                window.location.href = '{{ url("settings/payment/rentalPayStack")}}';
